            ...
            if len(group) == 1:
                ...
                if is_final_group:
                    ...
                    yield from self._learner.execute(
                        instruction, agents, model, system_instruction=system_instruction, context=last_result, stream=True, record_trace=False, hitl=hitl, priority=priority
                    )
                else:
                    # Intermediate: run silently with learning
                    last_result = self._learner.execute(
                        instruction, agents, model, system_instruction=system_instruction, context=last_result, stream=False, record_trace=False, hitl=hitl, priority=priority
                    )
            else:
